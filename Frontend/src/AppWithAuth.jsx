...
import HamburgerMenu   from './components/HamburgerMenu.jsx'  // ← new
import AuthPage        from './pages/AuthPage.jsx'             // ← new
import { useApp }      from './context/AppContext.jsx'
import { useAuth }     from './context/AuthContext.jsx'        // ← new
import { useProtectedAction } from './routes/ProtectedRoute.jsx' // ← new
...
/* ── Gate toast (shown briefly when auth is needed) ── */
...
function App() {
  ...
  const [showAuthPage,    setShowAuthPage]    = useState(false); // ← new
  const [gateToast,       setGateToast]       = useState(false); // ← new
  ...
  const { isAuthenticated, pendingAction, setPendingAction } = useAuth(); // ← new
  ...
  /* ── Auth success: close page, resume pending action ── */
  const handleAuthSuccess = useCallback(() => {
    setShowAuthPage(false);
    if (pendingAction === 'mic' || pendingAction === 'chat') {
      setIsChatOpen(true);
  ...
  const guardedOpenMic  = useProtectedAction('mic',  () => setIsChatOpen(true),    openRedirect);
  const guardedOpenChat = useProtectedAction('chat', () => { setIsChatOpen(true); setIsChatPanelOpen(true); }, openRedirect);
    ...
    <>
      ...
      <nav className={`navbar${isScrolled ? ' navbar--scrolled' : ''}`}>
        ...
        <div className="navbar-right">
          ...
          {/* ── HAMBURGER with auth state ── */}
          <HamburgerMenu onOpenAuth={handleOpenAuth} />
        </div>
      </nav>
      ...
      {!isChatOpen && (
        <div className="chatbot-container">
          <button
            ...
            onClick={guardedOpenChat}   /* ← guarded */
          >
